...
BEGIN
    ...
    if @rowcount_Table1=@rowcount_table2 then
            ...
            WHILE @counter <= @counterMAX DO
                ...
                SET @qry=CONCAT('select col INTO @columname from (SELECT @rownr:=@rownr+1 AS rowNumber, CONCAT(q.sid, "X", q.gid, "X", q.qid) as col FROM lime_questions AS q order by col ) as cols where rowNumber = ', @counter ,';');
                PREPARE `stmt` FROM @`qry`;
                EXECUTE `stmt`;
                DEALLOCATE PREPARE `stmt`;
                ...
                SET @qry=CONCAT('select CONCAT(', @question ,',": ","', @answer ,'") into @QandA;');
                PREPARE `stmt` FROM @`qry`;
                EXECUTE `stmt`;
                DEALLOCATE PREPARE `stmt`;   
            ...
            SET @qry=CONCAT('select * INTO OUTFILE "/var/lib/mysql-files/', @rowcount_Table1 ,'.csv" FIELDS TERMINATED BY "\n" from resp_tmp;');
            PREPARE `stmt` FROM @`qry`;
            EXECUTE `stmt`;
            DEALLOCATE PREPARE `stmt`;
